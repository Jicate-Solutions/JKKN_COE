...
export async function downloadCvPassPercentagePdf(input: CvPassPercentageInput): Promise<void> {
	try {
		console.log('[cv-pass-percentage-pdf] Starting PDF generation')
		...
		console.log('[cv-pass-percentage-pdf] Header drawn at y:', y)
		...
		console.log('[cv-pass-percentage-pdf] Creating table with', body.length, 'rows')
		autoTable(doc, {
			startY: y + 2,
			head: head as any,
			body,
		...
		console.log('[cv-pass-percentage-pdf] Table rendered')
		...
		console.log('[cv-pass-percentage-pdf] Footer drawn')
		...
		console.error('[cv-pass-percentage-pdf] Error:', error)
		throw error
	}
}
