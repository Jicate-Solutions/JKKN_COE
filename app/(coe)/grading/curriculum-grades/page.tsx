...
export default function CurriculumGradesPage() {
	...
	const filtered = useMemo(() => {
		...
		const data = items
			.filter((i) => [i.institutions_code, i.regulation_code, i.grade, i.classification, i.description, String(i.grade_point)].filter(Boolean).some((v) => String(v).toLowerCase().includes(q)))
	...
	const save = async () => {
		...
		try {
			...
			if (editing) {
				const res = await fetch('/api/grading/curriculum-grades', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editing.id, ...payload }) })
				if (!res.ok) {
				...
				const res = await fetch('/api/grading/curriculum-grades', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
				if (!res.ok) {
					const errorData = await res.json()
			...
			console.error(e)
			const errorMessage = e instanceof Error ? e.message : 'Failed to save curriculum grade'
			alert(errorMessage)
		} finally {
			setSaving(false)
		}
	...
	const remove = async (id: string) => {
		try {
			const res = await fetch(`/api/grading/curriculum-grades?id=${id}`, { method: 'DELETE' })
			if (!res.ok) throw new Error('Delete failed')
			setItems((p) => p.filter((x) => x.id !== id))
		} catch (e) {
			console.error(e)
			alert('Failed to delete curriculum grade')
		}
	}
	...
	const handleDownload = () => {
		...
		a.href = url
		a.download = `curriculum_grades_${new Date().toISOString().split('T')[0]}.json`
		...
		toast({
			...
			description: `${filtered.length} curriculum grades exported to JSON.`,
	...
	const handleExport = () => {
		...
		XLSX.utils.book_append_sheet(wb, ws, 'Curriculum Grades')
		XLSX.writeFile(wb, `curriculum_grades_export_${new Date().toISOString().split('T')[0]}.xlsx`)

		toast({
			title: '✅ Export Successful',
			description: `${filtered.length} curriculum grades exported to Excel.`,
			className: 'bg-green-50 border-green-200 text-green-800 dark:bg-green-900/20 dark:border-green-800 dark:text-green-200'
		})
	}
	...
	const handleTemplateExport = async () => {
		...
		// Export file
		XLSX.writeFile(wb, `curriculum_grades_template_${new Date().toISOString().split('T')[0]}.xlsx`)

		toast({
			title: '✅ Template Downloaded',
			description: 'Curriculum grades upload template with unified reference data has been downloaded successfully.',
			className: 'bg-green-50 border-green-200 text-green-800 dark:bg-green-900/20 dark:border-green-800 dark:text-green-200'
		})
	}
			...
			try {
				...
				for (let i = 0; i < mapped.length; i++) {
					...
					const payload = {
						...
						classification: (gradeItem as any).classification ?? '',
					...
					try {
						const response = await fetch('/api/grading/curriculum-grades', {
							method: 'POST',
						...
						if (response.ok) {
							...
							uploadErrors.push({
								...
								errors: [errorData.error || 'Failed to save curriculum grade']
							})
						}
					} catch (error) {
						errorCount++
				...
				if (successCount > 0 && errorCount === 0) {
					toast({
						...
						description: `Successfully uploaded all ${successCount} row${successCount > 1 ? 's' : ''} (${successCount} curriculum grade${successCount > 1 ? 's' : ''}) to the database.`,
						className: 'bg-green-50 border-green-200 text-green-800 dark:bg-green-900/20 dark:border-green-800 dark:text-green-200',
						duration: 5000,
					})
	...
	const fetchGrades = async () => {
		try {
			...
			const url = appendToUrl('/api/grading/curriculum-grades')
			...
			console.error(e)
			alert('Failed to fetch curriculum grades')
		} finally {
			setLoading(false)
		}
	}
	...
	const fetchRegulations = useCallback(async (institutionCode?: string) => {
		try {
			...
			console.error('[Curriculum Grades] Failed to fetch regulations from MyJKKN:', e)
		} finally {
		...
		<SidebarProvider>
			...
			<SidebarInset className="flex flex-col min-h-screen">
					...
					<div className="flex items-center gap-2">
						<Breadcrumb>
							<BreadcrumbList>
								...
								<BreadcrumbSeparator />
								<BreadcrumbItem>
									<BreadcrumbPage>Curriculum Grades</BreadcrumbPage>
								</BreadcrumbItem>
							</BreadcrumbList>
						</Breadcrumb>
					</div>
							...
							<CardContent className="p-4">
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-xs font-medium text-muted-foreground mt-0.5">Total Curriculum Grades</p>
									</div>
						...
						<CardHeader className="flex-shrink-0 px-4 py-3 border-b">
							<div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between">
								<div className="flex items-center gap-3">
									<div>
										<h2 className="text-base font-semibold font-grotesk">Curriculum Grades</h2>
										<p className="text-xs text-muted-foreground">Manage curriculum grade definitions and point mappings</p>
									</div>
								</div>
									...
									<Table>
										<TableHeader className="sticky top-0 z-10 bg-muted/50">
											<TableRow className="hover:bg-transparent">
												...
												<TableHead className="w-[160px] text-xs font-semibold"><Button variant="ghost" size="sm" onClick={() => handleSort("classification")} className="h-auto p-0 font-semibold hover:bg-transparent text-xs">Classification <span className="ml-1">{getSortIcon("classification")}</span></Button></TableHead>
										...
										<TableBody>
											{loading ? (
												<TableRow><TableCell colSpan={mustSelectInstitution ? 9 : 8} className="h-32 text-center">
													<div className="flex flex-col items-center gap-2 text-muted-foreground">
														<RefreshCw className="h-5 w-5 animate-spin" />
														<span className="text-sm">Loading curriculum grades...</span>
													</div>
												</TableCell></TableRow>
											) : pageItems.length ? (
												<>
												...
												<TableRow><TableCell colSpan={mustSelectInstitution ? 9 : 8} className="h-32 text-center">
													<div className="flex flex-col items-center gap-2 text-muted-foreground">
														...
														<span className="text-sm">No curriculum grades found</span>
														<span className="text-xs">Add a curriculum grade to get started</span>
													</div>
												</TableCell></TableRow>
											)}
										</TableBody>
				...
				<AlertDialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null) }}>
					<AlertDialogContent>
						<AlertDialogHeader>
							<AlertDialogTitle>Delete Curriculum Grade</AlertDialogTitle>
							<AlertDialogDescription>
								Are you sure you want to delete curriculum grade <strong>{deleteTarget?.grade}</strong> ({deleteTarget?.description})?
							</AlertDialogDescription>
						</AlertDialogHeader>
						<AlertDialogFooter>
				...
				<SheetContent className="sm:max-w-[720px] overflow-y-auto">
					<SheetHeader className="pb-4 border-b">
						<SheetTitle className="text-lg font-semibold font-grotesk">
							{editing ? "Edit Curriculum Grade" : "Add Curriculum Grade"}
						</SheetTitle>
						<p className="text-sm text-muted-foreground">
							{editing ? "Update curriculum grade information and settings" : "Define a new curriculum grade with point value and classification"}
						</p>
					</SheetHeader>
					...
					<div className="mt-6 space-y-8">
						...
						<div className="space-y-4">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								...
								<div className="space-y-2 md:col-span-2">
									...
									<Input value={formData.classification} onChange={(e) => setFormData({ ...formData, classification: e.target.value })} className="h-10" placeholder="e.g., First Class with Distinction, First Class, Second Class" />
								</div>
								<div className="space-y-2 md:col-span-2">
									<Label className="text-sm font-semibold">Description *</Label>
									<Textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className={`min-h-[80px] ${errors.description ? 'border-destructive' : ''}`} placeholder="Description of this curriculum grade" />
						...
						<div className="flex justify-end gap-3 pt-6 border-t">
							...
							<Button size="sm" className="h-10 px-6" onClick={save} disabled={saving}>
								{saving ? (editing ? 'Updating…' : 'Creating…') : (editing ? 'Update Curriculum Grade' : 'Create Curriculum Grade')}
							</Button>
						</div>
					</div>
				</SheetContent>
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<AlertDialogDescription className="text-sm text-muted-foreground mt-1">
									{importErrors.length === 0
										? 'All curriculum grades have been successfully uploaded to the database'
							...
							<div className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800 rounded-lg p-4">
								<div className="flex items-center gap-2">
									...
									<span className="font-semibold text-green-800 dark:text-green-200">
										All {uploadSummary.success} curriculum grade{uploadSummary.success > 1 ? 's' : ''} uploaded successfully
									</span>
								</div>
							</div>
						)}
							...
							<div className="flex items-start gap-2">
								...
								<div>
									...
									<ul className="text-xs text-blue-700 dark:text-blue-300 space-y-1">
										...
										<li>• <strong>Classification</strong> (optional): Classification label (e.g., First Class with Distinction)</li>
