				...
				<AppHeader>
					<Breadcrumb>
						<BreadcrumbList>
							...
							<BreadcrumbSeparator />
							<BreadcrumbItem>
								<BreadcrumbPage>Examiner Allotment</BreadcrumbPage>
							</BreadcrumbItem>
						</BreadcrumbList>
					</Breadcrumb>
				</AppHeader>
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground">
								Assign internal &amp; external examiners for practical / project exams
							</p>
						</div>
					</div>
					...
					{isReady && (
						<Tabs defaultValue="allotment" className="space-y-4">
							<TabsList className="grid w-full max-w-md grid-cols-2 bg-slate-100 dark:bg-slate-800/60 p-1 h-10 rounded-lg">
								<TabsTrigger value="allotment" className="text-xs gap-1.5 rounded-md data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm data-[state=active]:font-semibold dark:data-[state=active]:bg-slate-700 dark:data-[state=active]:text-white">
									<UserCheck className="h-3.5 w-3.5" />
									Examiner Allotment
								</TabsTrigger>
								<TabsTrigger value="report" className="text-xs gap-1.5 rounded-md data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm data-[state=active]:font-semibold dark:data-[state=active]:bg-slate-700 dark:data-[state=active]:text-white">
																		...
																		onSelect={(staff) => {
																			setRows(prev => prev.map(r =>
																				r.timetable_id === row.timetable_id
																					? { ...r, internal_examiner: { staff_id: staff.id, staff_name: staff.name, staff_mobile: staff.phone } }
																					: r
																			))
																...
																<TableCell className="py-1">
																	<SkilledExaminerCell
																		...
																		onSelect={(staff) => {
																			setRows(prev => prev.map(r =>
																				r.timetable_id === row.timetable_id
																					? { ...r, skilled_examiner: { staff_id: staff.id, staff_name: staff.name, staff_mobile: staff.phone } }
																					: r
																			))
																...
																<TableCell className="py-1">
																	<ProgrammerExaminerCell
																		...
																		onSelect={(staff) => {
																			setRows(prev => prev.map(r =>
																				r.timetable_id === row.timetable_id
																					? { ...r, programmer_examiner: { staff_id: staff.id, staff_name: staff.name, staff_mobile: staff.phone } }
																					: r
																			))
