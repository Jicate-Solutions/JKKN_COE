...
export function AllotmentReportTab({
	...
	const [registerRanges, setRegisterRanges] = useState<Record<string, RegisterRange>>({})
	...
	const fetchData = useCallback(async () => {
		...
		try {
			...
			// Fetch register ranges
			if (data.length > 0) {
				const timetableIds = data.map(r => r.timetable_id).join(',')
				const rangeRes = await fetch(`/api/pre-exam/examiner-allotment?action=register-ranges&timetableIds=${timetableIds}`)
				if (rangeRes.ok) {
					setRegisterRanges(await rangeRes.json())
				}
			}
		} catch {
	...
	const getRegisterRange = (ttId: string): string => {
		const range = registerRanges[ttId]
		if (!range) return '-'
	...
	const getSelectedGroups = () => {
		...
		// Format external: name, designation, institution + address, mobile
		const formatExternal = (e: ExternalExaminer | null): string => {
			if (!e) return ''
			const parts = [e.full_name]
			if (e.designation) parts.push(e.designation)
			// Build address: institution_name, institution_address or address+city+state+pincode
			const addrParts: string[] = []
			if (e.institution_name) addrParts.push(e.institution_name)
			if (e.institution_address) {
				addrParts.push(e.institution_address)
			} else {
				const loc = [e.address, e.city, e.state, e.pincode].filter(Boolean).join(', ')
		...
		const groups = groupOrder.map(code => ({
			...
			rows: groupMap.get(code)!.map(r => ({
				...
				register_range: getRegisterRange(r.timetable_id),
	...
	const handleDownloadPdf = async () => {
		...
		try {
			...
			try {
				const instRes = await fetch(`/api/master/institutions?institution_code=${institutionCode}`)
				if (instRes.ok) {
			...
			const doc = generateAllotmentReportPdf({
				institution_name: institutionName || 'J.K.K. Nataraja College of Arts & Science (Autonomous)',
		...
		try {
			...
			for (const group of groups) {
				for (const r of group.rows) {
					serial++
					const range = registerRanges[r.timetable_id]
					const vals: (string | number)[] = [
						serial,
											...
											return (
												...
												>
													...
													<TableCell className="text-xs py-2 font-mono">
														{getRegisterRange(row.timetable_id)}
													</TableCell>
													<TableCell className="text-xs py-2">
