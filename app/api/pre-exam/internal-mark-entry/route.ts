		...
		switch (action) {
			...
			case 'question-paper': {
				...
				const result = papers.map((p: any) => {
					// A question split into sub-divisions ("12 a) i. / ii.") contributes one
					// entry column per sub-division, each with its own marks and CO/K. The
					// parent's choice_group is kept, so the OR / "answer any N" rules below
					// still treat 12a-i and 12a-ii as one answered question.
					const questions = flattenEntryQuestions(p.questions)
	...
	try {
		...
		if (paperIds.length > 0) {
			...
			for (const p of (paperRows || [])) {
				...
				// Sub-divisions are entry targets in their own right; flattening keeps them
				// on the parent's choice group so OR / "answer any N" still hold.
				for (const q of flattenEntryQuestions(p.questions)) {
					const part = q.part_label ?? ''
					index.set(q.id, {
						part,
			...
			for (const lm of learner_marks) {
				...
				for (const entry of Object.values(lm.question_marks || {}) as any[]) {
					...
					if (!index) continue
					// group → branch id → labels answered on that branch. Only ONE branch of
					// an OR pair may be answered; a split branch may hold several columns.
					...
					for (const [, branches] of answeredByGroup) {
						if (branches.size > 1) {
							// One label per branch is enough to name the clash.
							const names = [...branches.values()].map(labels => labels[0])
							errors.push(`${regNo}: only one of Q${names.join(' / Q')} may be answered (OR choice)`)
						}
					}
