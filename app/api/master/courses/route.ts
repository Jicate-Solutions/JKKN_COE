...
CREATE INDEX IF NOT EXISTS idx_courses_created_at ON courses(created_at);
    ...
    const mapped = (data || []).map((row: any) => ({
      ...
      course_type_code: row.course_type_code ?? null,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase2.from('courses').insert({
      ...
      course_level: input.course_level ? String(input.course_level) : null,
      course_part_master: input.course_part_master ? String(input.course_part_master) : null,
      part_number: (input.part_number !== undefined && input.part_number !== null && String(input.part_number) !== '') ? Number(input.part_number) : null,
